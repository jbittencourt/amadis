		...
		switch($_REQUEST['comments_action']) {
			case "A_make_comment":
				try {
					...
					$comment->text = $_REQUEST['frm_desComentario'];
					$comment->time = time();
					$comment->save();

					try {
						$projComment = new AMProjectComment;
						$projComment->codeProject = $proj->codeProject;
						$projComment->codeComment = $comment->codeComment;
						$projComment->save();
					} catch (CMException $e) {
						$pag->addError($_language['error_not_save_comment'],$this);
						$comment->delete();
						break;
					}

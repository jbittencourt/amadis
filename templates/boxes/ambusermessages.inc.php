...
class AMBUserMessages extends AMColorBox {
	...
	public function __construct() {
		...
		try {
			...
			$this->cDiary = $_SESSION['user']->getLastBlogComments();
	...
	public function __toString() {
		...
		if($this->cProjects->__hasItems()) {
			...
			foreach($this->cProjects as $item) {
				...
				parent::add("<a class=\"new_comments\" href=\"$_CMAPP[services_url]/projetos/projeto.php?frm_codProjeto=$item->codeProject\" class=\"cinza\">");
    ...
    //listing the comments in the Diary
		if($this->cDiary->__hasItems()) {
			...
			foreach($this->cDiary as $item) {
				$men = $_language[diary_comments] ;
				if($item->numMessages == 1) {
					$men =  $_language[diary_comment];
				};
				parent::add('<a class="new_comments" href="'.AMBoxDiario::getPermanentLink($item).'" class="cinza">');
				parent::add("$item->numMessages $men</a> <b>$item->titulo</b><br>");

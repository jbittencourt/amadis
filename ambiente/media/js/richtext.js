...
function writeRichText(rte, html, width, height, buttons, readOnly) {
  if (isRichText) {
    ...
    if (buttons == true) {
      ...
      document.writeln('		<td><div id="forecolor_' + rte + '"><img class="rteImage" src="' + imagesPath + 'textcolor.gif" width="25" height="24" alt="Text Color" title="Text Color" onClick="dlgColorPalette(event,\'' + rte + '\', \'forecolor\', \'\')"></div></td>');
...
function enableDesignMode(rte, html, readOnly) {
  frameHtml = getFrameHTML(rte,html);
  initDesignMode(rte,readOnly);
}


function initDesignMode(rte,readOnly) {
  if (document.all) {
	...
	alert("Error preloading content.");
      ...
      if (isGecko) {
	setTimeout('initDesignMode("' + rte + '",  '+ readOnly +' );', 10);
      } else {
	return false;
      }
    }

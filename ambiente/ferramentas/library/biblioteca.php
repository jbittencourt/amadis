...
if($count_all != 0 ){
  for($i=0;$i<5;$i++){//sao 5 categorias de arquivos
	...
	if($ret != ""){
	  ...
	  $conteudo .= "<td class='blt_linha_tipo' width='50'>$_language[action]</td>";
	  $conteudo .= "</tr>";
	  foreach($ret as $item){
	    //arruma os icones..
	...
	if($ret != ""){
	  foreach($ret as $item){	      
	    ...
	    //<img src='".$_CMAPP[media_url]."/imagewrapper.php?method=db&frm_codeArquivo=".$item->codeArquivo."' width=50% height=50% alt='".$item->nome."'>

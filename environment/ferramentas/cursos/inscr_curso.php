...
if ($_REQUEST[acao] == "inscrever") {
	...
	echo "nao foi possivel salvar";
      ...
      $pag->add("<p align=center class=fontgray>Inscricao Aceita<br><br><a class=fontcolor href=\"curso.php?frm_codCurso=".$curso->codCurso."\">ir para o curso</a></p>");
    }
    else {
      $inscr->flagAutorizado =1;
      try{
...
elseif ($curso->datInscricaoFim < time()) {
  ...
  if ($req->flagAutorizado==2) {
    $tab->add ("<p align=center class=fontcolor>Inscricao jah rejeitada&nbsp;".date("d/m/Y",$req->tempo)." hshs $lang[inscr_jah_rejeitada2]</p>");
  }

...
if (empty($_REQUEST[frm_codEscola])) die ("Erro fatal");

$escola = new AMEscola($_REQUEST[frm_codEscola]);

if ($_SESSION[usuario]) die ("Permissao negada");

$admins = $escola->listaAdmins();

//cria o sub-menu
//se nao tiver admin selecionado
  if (empty($_REQUEST[frm_codAdmin]) or ($_REQUEST[acao] == "A_apaga") or ($_REQUEST[acao] == "A_salvar")) {

    //verifica a url pra depois de salvar os dados redirecionar o usuario para lah
    $itens[$lang[voltar_escola]] = "$urlferramentas/comunidade/escola.php?frm_codEscola=".$escola->codEscola;
    $itens[$lang[promover_novos_admins]] = "escola_admin.php?frm_codAdmin=new";
  }
